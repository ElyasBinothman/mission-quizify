...
class QuizGenerator:
    ...
    def init_llm(self):
        ...
        This method prepares the LLM for generating quiz questions by configuring essential parameters such as
        the model name, temperature, and maximum output tokens.
        """
        self.llm = VertexAI(
    ...
    def generate_question_with_vectorstore(self):
        """
        Generates a quiz question using the topic provided and context from the vectorstore.

        This method leverages the vectorstore to retrieve relevant context for the quiz topic, then uses the LLM to generate
        a structured quiz question in JSON format.
        """
        if not self.llm:
            self.init_llm()
